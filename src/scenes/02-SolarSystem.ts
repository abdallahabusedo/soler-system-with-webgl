...
export default class SolarSystemScene extends Scene {
    ...
    private drawSystem(parent: mat4, system: SolarSystemDescription){
        ...
        for (let i = 0 ; i < system.children.length ; i++)
        {
            ...
            let x = system.children[i].distanceFromParent * Math.cos(this.time*system.children[i].rotationSpeedAroundParent);
            let z = -system.children[i].distanceFromParent * Math.sin(this.time*system.children[i].rotationSpeedAroundParent);
